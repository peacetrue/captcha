...
@Setter
@AllArgsConstructor
public class TapCaptchaService extends AbstractCaptchaService<Point> {

    private TapBufferedImageSupplier bufferedImageSupplier;

    public TapCaptchaService(CaptchaRepository<Point> captchaRepository, Long expirePeriod,
                             Integer maxFailedCount, TapBufferedImageSupplier bufferedImageSupplier) {
    ...
    public QuestionAnswer<Serializable, Point> generateQuestionAnswer(CaptchaQuestionOptions options) {
        ...
        Set<String> words = randomWords(4);
//        BufferedImageUtils.buildBufferedImage()
...
//        return new QuestionAnswer<>(new TapQuestion(
...
//        ), answer);
    ...
    private static char randomChar() {
        return HAN_ZI.charAt(RandomUtils.nextInt(0, HAN_ZI.length()));
    }

    private static Set<String> randomWords(int wordCount) {
        Set<String> words = new HashSet<>();
        while (words.size() < wordCount) {
            char chars = HAN_ZI.charAt(RandomUtils.nextInt(0, HAN_ZI.length()));
